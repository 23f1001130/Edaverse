...
export default function HistoryPanel({ onSelect }) {
  ...
  return (
    <div className="ws-panel">
      ...
      <div className="hist">
        {loading ? <div className="tab-empty">Loading…</div>
        : !datasets.length ? <div className="tab-empty">No datasets yet — upload a file to get started.</div>
        : datasets.map(ds => (
          <div key={ds.id} className="hist-item" onClick={()=>onSelect(ds.id)}>
            ...
            <div className="hist-info">
              ...
              <div className="hist-meta">{ds.shape?.rows?.toLocaleString()} rows · {ds.shape?.columns} cols · {timeAgo(ds.saved_at)}</div>
            </div>
